...
from aut.models import various_fields
from typing import Type, Optional as O, Union as U, Any


class FieldTests(TestCase):
    module_path = 'aut.models.various_fields'
    entry_bases = (various_fields.HeadlinedEntry,)
    mongo_field_model_container = various_fields.NamedBlog
    mongo_field = models.EmbeddedField
    blog_value: U[dict, list, None]
    headline_query: dict
    blog_query: dict
    mut: U[Type[models.Model], type]
    entry: various_fields.EmbeddedFieldEntry
    db_entry: various_fields.EmbeddedFieldEntry
    ...
    def model_constructor_kwargs(self):
        ...
        kwargs = {
            ...
            'nested_blog': self.mongo_field(
                model_container=various_fields.NamedBlog,
...
class TestReference(TestCase):
    ...
    def test_create(self):
        e1 = various_fields.ReferenceEntry.objects.create(
            headline='h1',
        )
        e2 = various_fields.ReferenceEntry(headline='h2')
        e2.save()

        a1 = various_fields.ReferenceAuthor.objects.create(
            name='n1',
            email='e1@e1.com'
        )
        a2 = various_fields.ReferenceAuthor.objects.create(
            name='n2',
            email='e2@e2.com'
        )

        self.assertEqual([], list(e1.authors.all()))
        self.assertEqual([], list(a1.various_fields.ReferenceEntry_set.all()))

        e1.authors.add(a1)
        self.assertEqual(e1.authors_id, {a1.pk})
        self.assertEqual([a1], list(e1.authors.all()))
        self.assertEqual([e1], list(a1.various_fields.ReferenceEntry_set.all()))

        e2.authors.add(a1, a2)
        self.assertEqual(e2.authors_id, {a1.pk, a2.pk})
        self.assertEqual([a1, a2], list(e2.authors.all()))
        self.assertEqual([e1, e2], list(a1.various_fields.ReferenceEntry_set.all()))
        self.assertEqual([e2], list(a2.various_fields.ReferenceEntry_set.all()))

        g = various_fields.ReferenceEntry.objects.get(headline='h1')
        self.assertEqual(e1, g)
        g = various_fields.ReferenceEntry.objects.get(authors__name='n2')
        self.assertEqual(e2, g)
        g = list(various_fields.ReferenceEntry.objects.filter(authors__name='n1'))
        self.assertEqual([e1, e2], g)

        a2.various_fields.ReferenceEntry_set.add(e1)
        self.assertEqual(e1.authors_id, {a1.pk, a2.pk})
        self.assertEqual([e1, e2], list(a2.various_fields.ReferenceEntry_set.all()))
...
class TestCRUD(TestCase):
    tags = ('basic', 'mongodb_field', 'crud')

    def test_crud(self):

        b1 = various_fields.BasicBlog.objects.create(
            name='b1',
            tagline='t1'
        )
        b2 = various_fields.BasicBlog.objects.create(
            name='b2',
            tagline='t2'
        )
        e1 = various_fields.BasicRelatedEntry.objects.create(
            headline='h1',
            blog=b1
        )
        e2 = various_fields.BasicRelatedEntry.objects.create(
            headline='h2',
            blog=b2
        )
        a1 = various_fields.BasicAuthor.objects.create(
            name='a1'
        )
        a2 = various_fields.BasicAuthor.objects.create(
            name='a2'
        )
        self.assertEqual([], list(e1.authors.all()))
        ...
        g = various_fields.BasicRelatedEntry.objects.get(headline='h1')
        self.assertEqual(e1, g)
        g = various_fields.BasicRelatedEntry.objects.get(authors__name='a2')
        self.assertEqual(e2, g)
        g = list(various_fields.BasicRelatedEntry.objects.filter(authors__name='a1'))
        self.assertEqual([e1, e2], g)
...
class TestRelations(TestCase):
    ...
    def test_join(self):
        ...
        eqs = various_fields.BasicRelatedEntry.objects.filter(blog__name='b1').values('id')
        bqs = various_fields.BasicBlog.objects.filter(id__in=eqs).values('name')
